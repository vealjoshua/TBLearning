...
try {
    ...
    Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
    Statement stmt = conn.createStatement();
    String this_query = "insert into course (Course_code, Course_Desc, Course_year, Course_Semster, Course_Time, Course_Department_ID, Course_UserID, Course_Deleted) values('"+Course_code+"','"+Course_Desc+"',"+Course_year+",'"+Course_Semster+"','"+Course_Time+"','"+Course_Department_ID+"','"+Course_UserID+"', "+Course_Deleted+");";
    stmt.execute(this_query);
    stmt.close(); 
    conn.close();
      } catch (ClassNotFoundException | SQLException e) {
